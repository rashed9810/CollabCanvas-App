...
const SendIcon = ({ className }: { className?: string }) => (
  <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19.8V2.216a1 1 0 00-1.597.655l-2.363 5.625a1 1 0 00.309 1.165l4.236 3.389a1 1 0 01.309.915V21.4l6.188-3.869A1 1 0 0020 17v-8a1 1 0 00-1-1H3a1 1 0 01-1-1v-4a1 1 0 011-1h16a1 1 0 011 1v4a1 1 0 01-1 1h-13.5m-6.5 11l6-6m0 0l-6-6m6 6H3" />
  </svg>
);

const ChatIcon = ({ className }: { className?: string }) => (
  <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M12 16a4 4 0 100-8 4 4 0 0108z" />
  </svg>
);
...
const ChatSidebar: React.FC<ChatSidebarProps> = ({
  ...
  const renderChatToggle = () => {
    const baseClasses = "fixed bottom-5 right-5 w-14 h-14 rounded-full flex items-center justify-center text-white shadow-lg transition-all duration-300 focus:outline-none";
      ...
      >
        {isOpen ? (
          ...
          <div className="relative">
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-2 -right-1 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center text-xs">
          ...
          {messages.length === 0 ? (
            ...
            <div className="space-y-4">
              {messages.map((message, index) => (
                <div 
                  key={index} 
                  className={`mb-4 ${message.userId === user?._id ? 'text-right' : ''}`}
                >
                  <div 
